...
st.info("💡 **AI 학습 벤치마크:** 과거 **'경의선 및 경의중앙선' 지상 구간이 지하화되고 숲길로 조성된 후, 낮 시간 여가 유입과 심야 상권이 폭발적으로 증가했던 실제 성공 사례**를 머신러닝이 학습하여 미래를 예측합니다.")
st.markdown("---")
...
with st.expander("📊 AI가 학습한 과거 성공 사례 (경의선 숲길 트렌드 동적 추출)", expanded=True):
    st.markdown("업로드된 지하철 원본 데이터에서 **2015년~2018년 경의선 숲길 수혜 역**의 실제 성장 트렌드를 실시간으로 추출한 결과입니다. (※ 시각적 왜곡을 방지하기 위해 유동인구가 지나치게 거대한 홍대입구역은 차트에서만 제외했습니다.)")
    ...
    if not bench_df.empty:
        ...
        st.warning("업로드된 데이터에 2015~2018년 경의선 데이터가 부족하여 벤치마크를 추출할 수 없습니다.")
...
if station_data.empty:
    st.error(f"'{target_station}' 역의 데이터를 찾을 수 없습니다.")
    st.stop()
...
st.write(f"현재 기초 낮 하차 비중: **{base_midday:.1f}%** (이 비중이 높을수록 지하화 후 파급력이 커집니다.)")
...
st.write(f"노원역 학원가 하원 인파 및 야간 상권 체류 인구를 나타냅니다. 이 수치가 높으면 팝업/공원 조성 시 사람들이 늦게까지 동네에 체류하게 됩니다.")
...
st.markdown(f"""
* **과거 경의중앙선의 교훈:** 단순히 철도를 땅에 묻는 것이 아니라, 과거 경의선 숲길처럼 **기존에 잠재력을 갖춘 동네(낮/심야 기초체력)가 환경 개선 버프를 받았을 때** 유동인구가 폭발하는 법칙을 따릅니다.
* **낮과 밤의 시너지:** {target_station}역은 현재 **{base_night:.1f}%**라는 훌륭한 20시 이후 심야 체류 데이터를 보유하고 있습니다. 지하화가 완료될 경우, 낮에 유입된(+{pred_growth:.1f}%) 인구가 심야 상권까지 이어지는 메가 상권으로 거듭날 수학적 근거가 충분합니다.
""")
...
st.markdown("AI가 경의선 사례를 바탕으로 인프라 충격을 예측할 때 가장 중요하게 본 데이터 가중치입니다.")
